...
import gopher10.util.ArrayUtils;
...
public class Launcher extends Robot {
    ...
    private RobotInfo getHighestPriorityTarget() throws GameActionException {
        RobotInfo[] attackableEnemies = rc.senseNearbyRobots(RobotType.LAUNCHER.actionRadiusSquared, team.opponent());
        int healthOfBest = 10000;
        int bestPreference = 10;
        RobotInfo bestEnemy = null;
        for (RobotInfo enemy : attackableEnemies) {
            if (attackPreference[enemy.getType().ordinal()] < bestPreference) {
                bestEnemy = enemy;
                bestPreference = attackPreference[enemy.getType().ordinal()];
            } else if (attackPreference[enemy.getType().ordinal()] == bestPreference && enemy.health < healthOfBest) {
                bestEnemy = enemy;
                bestPreference = attackPreference[enemy.getType().ordinal()];
                healthOfBest = enemy.health;
            }
        }
        return bestEnemy;
    }
    ...
    private boolean wouldMoveAllowTargetlessEnemyToAttack(Direction moveDirection) throws GameActionException {
        MapLocation locationAfterMove = rc.getLocation().add(moveDirection);
        RobotInfo[] sensedNearbyFriendlies = rc.senseNearbyRobots(rc.getLocation(), RobotType.LAUNCHER.visionRadiusSquared, team);

        outer: for (int i = sensedEnemyLaunchersStackPointer ; i-- > 0 ; ) {
            MapLocation enemyLoc = sensedEnemyLaunchers[i].getLocation();
            if (locationAfterMove.distanceSquaredTo(enemyLoc) > RobotType.LAUNCHER.actionRadiusSquared) {
                continue;
            }
            for (RobotInfo friendly : sensedNearbyFriendlies) {
                if (friendly.getLocation().isWithinDistanceSquared(enemyLoc, RobotType.LAUNCHER.actionRadiusSquared)) {
                    continue outer;
                }
            }
            return true;
    ...
    public int attack() throws GameActionException {
        ...
        if (highestPriority != null
                && rc.canAttack(highestPriority.getLocation())) {
            makeAttack(highestPriority);
            pathing.moveTowards(highestPriority.getLocation().directionTo(rc.getLocation()));
            turnsStoodby = 0;
        } else {

            Direction moveTowardsEnemyDirection = null;
            if (sensedEnemyLaunchersStackPointer != 0) {
                moveTowardsEnemyDirection = pathing.getBestMoveTowards(rc.getLocation().directionTo(sensedEnemyLaunchers[0].getLocation()));
            }
            if (moveTowardsEnemyDirection != null
                    && !wouldMoveAllowTargetlessEnemyToAttack(moveTowardsEnemyDirection)) {
                if (rc.canMove(moveTowardsEnemyDirection)) {
                    rc.move(moveTowardsEnemyDirection);
                }
                highestPriority = getHighestPriorityTarget();
                if (highestPriority != null
                        && rc.canAttack(highestPriority.getLocation())) {
                    makeAttack(highestPriority);
                    turnsStoodby = 0;
